		...
		struct hid_report *report, u8 *data, int size)
{
	...
	} else if (input->id.product == USB_DEVICE_ID_APPLE_MAGICMOUSE2) {
		/* Magic Mouse 2: only handle buttons here
		 * Touch scroll is handled in magicmouse_emit_touch via TOUCH_STATE_DRAG
		 */
		magicmouse_emit_buttons(msc, clicks & 3);
	} else if (input->id.product == USB_DEVICE_ID_APPLE_MAGICTRACKPAD) {
